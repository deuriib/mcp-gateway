...
def _handle_validation_error(
    ...
    if "url" in detail_lower and payload.get("type") == "remote":
        ...
        if _is_htmx_request(request):
            return HTMLResponse(
                f"<div class='toast bg-red-100 p-2'>{_e(detail)}</div>", status_code=400
            )
...
def _check_local_gating(
    ...
    if (
        ...
        if _is_htmx_request(request):
            return HTMLResponse(
                f"<div class='toast bg-red-100 p-2'>{_e(detail)}</div>", status_code=403
            )
...
def _check_duplicate(
    ...
    if config.name in existing_names or json_exists or pyi_exists:
        ...
        if _is_htmx_request(request):
            return HTMLResponse(
                f"<div class='toast bg-red-100 p-2'>{_e(msg)}</div>", status_code=409
            )
...
async def handle_create(request: Request) -> JSONResponse | HTMLResponse:
    ...
    if err:
        return err
    return _create_success_resp(registry, config, tools or [], request)
