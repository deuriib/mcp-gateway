...
- **feat(cli)**: expose `mgw` as a 1:1 shortcut for `mcp-gway` (same `cli.main`, SPEC-MGW-001); `mcp-gway` stays canonical, both shims ship on install.
- **docs(perf)**: N/A — internal-only performance audit tooling (SPEC-PERF-001, engineering). Benchmark script, runbook, findings template, architecture NFRs, ADR-011. No user-facing impact, no src/ changes.
